#!/usr/bin/env/python
# modified Microsoft chem GGNN code from : https://github.com/Microsoft/gated-graph-neural-network-samples
"""
Usage:
...
class DenseGGNNPLModel(PLModel):
    ...
    def process_raw_graphs(self, d: Sequence[Any], is_training_data: bool, bucket_sizes=None) -> Any:
        # Figure out what embedding size is & what our correspondent to active nodes is
        # create adjacency matrix -- figure out how many edge types & whether it is directed or undirected (look up what the tie_fwd_bkwd means in original paper)
        # modify this function & modify adj_mat function
        # correct anywhere that loops through buckets -- for now should just be one bucket I think but should eventually generalize it to work with mini-batches
        if bucket_sizes is None:
            bucket_sizes = np.array(list(range(0,self.max_num_vertices-1,4)) + [self.max_num_vertices])
        bucketed = defaultdict(list)
        x_dim = self.annotation_size
        chosen_bucket_idx = len(bucket_sizes)-1 
        chosen_bucket_size = bucket_sizes[chosen_bucket_idx]
        print('chosen bucket idx', chosen_bucket_idx)
        print('chosen bucket size', chosen_bucket_size)
        n_active_nodes = len(d["labels"])

        mask = np.zeros(self.max_num_vertices)
        for n in d["labels"]:
            mask[n["node"]] = 1
        bucketed[chosen_bucket_idx].append({
            'adj_mat': graph_to_adj_mat(d, chosen_bucket_size, self.num_edge_types, self.params['tie_fwd_bkwd']),
            'init': [[0 for _ in range(x_dim)] for __ in
                                              range( n_active_nodes)],
            'labels': [n["label"] for n in d["labels"]],
            'mask': mask
            })
        print(mask)
        if is_training_data:
