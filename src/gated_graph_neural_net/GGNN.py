...
class PLModel(object):
    ...
    def default_params(cls):
        return {
            ...
            'train_file': '../data/example.json',
            'valid_file': '../data/example.json'
        }
    ...
    def load_data(self, file_name, is_training_data: bool):
       ...
       # for g in data:
        for n in g["nodes"]:
            self.max_num_vertices = max(self.max_num_vertices, n['id']) + 1
            self.annotation_size = max(self.annotation_size, n['ast_type']) + 1

        num_fwd_edge_types = max(num_fwd_edge_types, max([e['edge_type'] for e in g['edges']])) + 1
        self.num_edge_types = max(self.num_edge_types, num_fwd_edge_types * (1 if self.params['tie_fwd_bkwd'] else 2))
        print('max vertices', self.max_num_vertices)
        print("annotation size", self.annotation_size)
        print("num forward edge types", num_fwd_edge_types)
        return self.process_raw_graphs(g, is_training_data)
